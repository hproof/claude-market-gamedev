...
1. 读取匹配的文档
2. 判断文档内容是否足以回答用户问题：
   - **足够** → 基于文档内容回答，引用来源：`（参考：[{doc_name}](./docs/code-analyzer/{doc_name})）`
   - **不足**（文档存在但缺少所问的具体信息） → 告知用户现有文档未覆盖该细节，询问是否对相关模块重新执行分析；用户确认后转入[分析流程](#分析流程) A2 步骤，分析完成后读取新文档回答原始问题
...
```
该模块尚未进行{type}分析，无法直接回答。是否先执行分析？
分析范围：{scope}  分析类型：{type}
```

4. 如用户确认：
   a. 以推导出的 scope 和 type 作为参数，转入[分析流程](#分析流程)的 A2 步骤执行分析
   b. 分析完成后（A8 结束），**读取新生成的分析文档**
   c. 基于文档内容**回答用户的原始问题**，引用文档来源
5. 如用户拒绝 → 结束
