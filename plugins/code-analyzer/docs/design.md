...
| **约束 7** | 分析优先通过 Skill 执行：代码分析范畴内的需求，优先通过三个分析 Skill 之一执行 | 用户意图为"分析"且属于代码分析范畴时 | 选择 `full-scan`/`module-analyzer`/`feature-analyzer` 之一调用；非代码分析范畴的直接告知无法处理 | 确保代码分析经过统一的约束检查，同时避免滥用 |
